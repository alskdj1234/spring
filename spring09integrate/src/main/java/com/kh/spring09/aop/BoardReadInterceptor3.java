...
//목표 : 
//- 세션(HttpSession)을 이용해서 읽은 글의 번호를 관리
//	(1) 세션에 memory라는 이름의 저장소가 있다고 가정한다 (저장소는 HashSet이다)
//	(2) 세션에서 memory 저장소를 꺼낸다
//	(3) 2번에서 저장소가 없으면 신규 생성한다
//	(4) 현재 읽으려는 글번호가 memory 저장소에 존재하는지 확인
//	(5-1) 만약 존재한다면 조회수 증가 처리 없이 통과
...
public class BoardReadInterceptor3 implements HandlerInterceptor{
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler)
		...
		//[3] 세션의 memory 항목을 조사하여 조회수 증가/유지 여부를 판정
		...
		//확인용 출력
		//System.out.println("읽은글 = " + memory);
		
		if(memory.contains(boardNo)) {//저장소에 존재하는 번호라면 (=이미 읽은적이 있는 글이라면)
