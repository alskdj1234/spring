...
public class AuthRestController {
	...
	public ResponseEntity<AuthLoginResponseVO> login(// 데이터+헤더+쿠키를 반환
		...
		ResponseCookie accessCookie = ResponseCookie.from("accessToken", accessToken)
				// 각종 설정들
				.maxAge(Duration.ofSeconds(jwtProperties.getAccessTokenValidity()))// 유효시간 설정
				.path("/")// 적용범위
		...
		ResponseCookie refreshCookie = ResponseCookie.from("refreshToken", refreshToken).maxAge(Duration.ofSeconds(// 유효시간
																													// 설정
																													// (JWT와
																													// 동일하게)
				jwtProperties.getRefreshTokenValidity())).path("/service/auth/")// 정확하게 갱신매핑에서만 사용되도록
				.httpOnly(true).secure(false).sameSite("Lax").build();
		// 리프레쉬 토큰 정보를 디비에 저장
		//유저 에이전트가 없을때 값을 언노운으로 세팅
		...
		accountRefreshDao.insertOrUpdate(
				AccountRefreshDto.builder().accountId(response.getAccountId()).tokenValue(refreshToken).userAgent(userAgent).build());
		// account_login 정보 갱신 들어갈 수 있음

		// 결과 반환
		return ResponseEntity.ok()
				// 쿠키를 추가하는 설정
				.header(HttpHeaders.SET_COOKIE, accessCookie.toString(), refreshCookie.toString()).body(response);
	}
	...
	public ResponseEntity<Void> logout(@CookieValue(name = "accessToken", required = false) String accessToken,
		...
		// db에서 저장된 refresh token 정보를 삭제
		// accessToken 검사 => refreshToken검사 => 넘기기
		try {
			if (accessToken != null) {
...
//			문제 생겨도 넘기기
	...
	public ResponseEntity<AuthLoginResponseVO> refresh(
		...
		//실제 db에 존재하는 토큰과 사용자가 가져온 토큰의 일치 여부를 검사
		// 만약 안 맞으면 whoareyou..
		AccountRefreshDto accountRefreshDto = accountRefreshDao.find(
				...
				.userAgent(userAgent)
				.build());
		if(accountRefreshDto == null) {//발급한적이 없어요
			throw new WhoAreYouException();
		}
		...
		ResponseCookie refreshCookie = ResponseCookie.from("refreshToken", newRefreshToken)// 새로만든 Refresh Token
		...
		// 리프레쉬 토큰 정보를 디비에 저장
				accountRefreshDao.insertOrUpdate(
						AccountRefreshDto.builder().accountId(accountId).userAgent(userAgent).tokenValue(newRefreshToken)
						.build());
